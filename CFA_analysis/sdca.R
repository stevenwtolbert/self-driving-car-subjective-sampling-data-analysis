...
library('semPlot')
library('semTools')
global_items <- read.csv('/Users/stolbert/code/self-driving-car-subjective-sampling-data-analysis/data-cleaning/scales/sdca_global.csv')
...
colnames(global_items) <- c(1:10)
colnames(global_items) <- paste("x",colnames(global_items), sep = '')
model <- 'SDCA=~ x1+x2+x3+x4+x7+x8+x9
x1~~x9
x3~~x8
x2~~x9
'
